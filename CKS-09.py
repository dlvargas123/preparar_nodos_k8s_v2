...
def run_validation():
    ...
    with open(REPORT_FILE, "w") as report:
        ...
        report.write(f"CONTROL: Validar --authorization-mode (RBAC Check)\n")
        report.write(f"ENTORNO: Rancher RKE2 v1.32.13\n")
        report.write("="*60 + "\n\n")

        try:
            # 1. Verificar presencia y valor del flag
            ...
            if TARGET_FLAG in process_output:
                ...
                if EXPECTED_VALUE in current_value:
                    status = f"PASÓ: El modo de autorización es seguro ({current_value})."
                else:
...
if __name__ == "__main__":
    if os.geteuid() != 0:
        print("Se requiere privilegios de ROOT para inspeccionar el API Server.")
    else:
        run_validation()

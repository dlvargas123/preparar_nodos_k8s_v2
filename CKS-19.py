...
TARGET_FLAG = "--client-ca-file"
CHECK_COMMAND = "ps -ef | grep kube-apiserver | grep -v grep"
REPORT_FILE = f"evidencia_client_ca_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

def run_validation():
    print(f"Iniciando validación de Client CA File en RKE2...")
    
    with open(REPORT_FILE, "w") as report:
        report.write(f"REPORTE DE AUDITORÍA DE SEGURIDAD - {datetime.now()}\n")
        report.write(f"CONTROL: Configurar --client-ca-file (mTLS validation)\n")
        report.write(f"ENTORNO: Rancher RKE2 v1.32.13\n")
        report.write("="*60 + "\n\n")

        try:
            # 1. Verificar presencia del flag en el proceso activo (Punto 4)
            ...
            if TARGET_FLAG in process_output:
                ...
                status = "PASÓ: El flag --client-ca-file está configurado correctamente."
                detail = f"Ruta de CA detectada: {ca_path}"
            else:
                status = "ADVERTENCIA: El flag no se encuentra explícito en el comando de ejecución."
                detail = "RKE2 gestiona la confianza de certificados mediante parámetros internos de seguridad."

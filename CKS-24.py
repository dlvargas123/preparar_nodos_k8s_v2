...
def audit_rbac_sa():
    ...
    with open(REPORT_FILE, "w") as report:
        ...
        report.write("\n--- 2. SERVICEACCOUNTS CON AUTOMOUNT HABILITADO (POR NAMESPACE) ---\n")
        ...
        if sas:
            ...
            for sa in sas.get("items", []):
                ...
                if automount is not False:
                    ...
                    # Ignorar namespaces del sistema RKE2 para reducir ruido en el reporte
                    if ns not in ["kube-system", "kube-public", "cattle-system", "rke2-system"]:
                        report.write(f"Namespace: {ns} | SA: {name} | Automount: ENABLED\n")
                        count += 1
            report.write(f"\nTotal ServiceAccounts fuera de sistema con Automount habilitado: {count}\n")
